...
int MessageRead::ReadSocket(int i_SocketFD, int i_TimeoutMS, MRH_Uint8* p_Buffer) noexcept
{
    ...
    {
        case -1:
            MRH_PSBLogger::Singleton().Log(MRH_PSBLogger::ERROR, "Socket polling failed for socket " +
    ...
    if (ss_Read < 0)
    {
        ...
        MRH_PSBLogger::Singleton().Log(MRH_PSBLogger::ERROR, "Socket read failed for socket " +
                                                             std::to_string(i_SocketFD) +
                                                             ": " +
                                                             std::string(std::strerror(errno)) +
                                                             " (" +
    ...
    {
        MRH_PSBLogger::Singleton().Log(MRH_PSBLogger::WARNING, "0 bytes read but polling succeeded for socket " +
                                                               std::to_string(i_SocketFD),
                                       "MessageStream.cpp", __LINE__);
        return -1;
    }
    ...
    {
        MRH_PSBLogger::Singleton().Log(MRH_PSBLogger::WARNING, "Invalid byte count read for socket " +
                                                               std::to_string(i_SocketFD),
                                       "MessageStream.cpp", __LINE__);
        return -1;
    }
